...
def tweet_pack(django_pack):
    ...
    # Create preview
    pp = PackPreview(django_pack.nsfw)
    for sticker in pack_sst_cli.stickers:
        pp.add_image(sticker.image_data)
    pp.create()
...
➡️ https://signalstickers.com/pack/{pack_sst_cli.id}{content_hashtag}
    ...
    media_ids = []
    res = twitter_api.media_upload("preview_light.png", file=pp.preview_light)
    media_ids.append(res.media_id)

    res = twitter_api.media_upload("preview_dark.png", file=pp.preview_dark)
    media_ids.append(res.media_id)
